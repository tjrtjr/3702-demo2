...
去掉 `--evaluate` 会重新训练并保存到默认模型路径。`--epochs` 指定轮数；`--checkpoint` 可指定单独的模型文件，避免覆盖已有结果。默认训练轮数分别为 VAE 30、UNet 30、GAN 50；本次实际训练轮数见下表。

| 模型 | 默认模型文件 | 主要结果文件 | 本次训练与结果 |
|---|---|---|---|
| VAE | `results/part4.pt` | `results/part4_reconstructions.png`、`results/part4_manifold.png` | CPU 30 轮，测试 BCE+KL/图像 1088.193 |
| UNet | `results/part4_2.pt` | `results/part4_2.png`，终端输出逐类 DSC | A100 12 轮，测试四类 DSC 均 >0.9 |
| GAN | `results/part4_3.pt` | `results/figures/part4_3/` 图像；`results/part4_3.json` 损失历史 | A100 30 轮，约 283 秒 |
...
当前脚本和 UNet/GAN 模型位于集群 `~/comp3710_lab2_20260910/`。在登录节点提交作业，GPU 训练由 SLURM 分配计算节点：
...
/opt/slurm/bin/sbatch --account=comp3710 --partition=comp3710 \
  ...
  --wrap='"$HOME/miniconda3/envs/torch/bin/python" -u src/comp3710_lab2/part4_2.py --data-dir /home/groups/comp3710/OASIS --checkpoint results/part4_2.pt'
```

VAE/GAN 训练将脚本、模型名、作业名和日志名对应改成 `part4.py` / `part4.pt` / `vae` 或 `part4_3.py` / `part4_3.pt` / `gan`。单轮检查可改用 `--partition=a100-test --time=00:20:00`，在 Python 命令后加 `--epochs 1`，并指定独立模型路径。`a100-test` 每个作业最长 20 分钟；排队时间随资源变化。

```bash
/opt/slurm/bin/squeue -u "$USER"
tail -f runs/unet-实际作业号.log

# 在 GPU 上加载已保存的 UNet，输出测试集 DSC 和分割图
/opt/slurm/bin/srun --account=comp3710 --partition=a100-test \
  --gres=gpu:a100:1 --cpus-per-task=4 --time=00:20:00 \
  "$HOME/miniconda3/envs/torch/bin/python" src/comp3710_lab2/part4_2.py \
  --data-dir /home/groups/comp3710/OASIS --evaluate --checkpoint results/part4_2.pt
```

模型和图像自动保存在集群项目的 `results/`，上述训练日志保存在 `runs/`。本次结果已下载到本机。以后取回新实验时，在本机项目根目录执行：
...
- Part 1 要求实际比较 GPU 张量 DFT、NumPy 朴素 DFT 和 NumPy FFT，并改变数据大小；没有 CUDA 时脚本会留下待测项。本次 A100 实测已完成，见 `results/metrics/part1_gpu.log`。
